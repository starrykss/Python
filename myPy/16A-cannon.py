...
def turn_up():          # ↑키를 눌렀을 때 호출되는 함수
    t.left(2)           # 거북이를 왼쪽으로 2도 돌립니다.

def turn_down():        # ↓키를 눌렀을 때 호출되는 함수
...
t.onkeypress(turn_up, "Up")     # ↑키를 누르면 turn_up 함수를 실행합니다.
t.onkeypress(turn_down, "Down")  # ↓키를 누르면 turn_down 함수를 실행합니다.
